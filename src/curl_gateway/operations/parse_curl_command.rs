...
pub fn parse_curl_command(script: &str) -> Result<CurlCommand, CurlGatewayError> {
    ...
    if !curl_command.starts_with("curl ") {
        return Err(CurlGatewayError::new(
            ErrorKind::Parsing,
            "parse_curl_command",
            "No curl command found in script"
        ));
    }
...
fn parse_method(command: &str) -> Result<HttpMethod, CurlGatewayError> {
  let method_regex = Regex::new(r"-X\s+(\w+)").unwrap();
  method_regex
      .captures(command)
      .map(|cap| HttpMethod::from_str(&cap[1].to_uppercase()))
      .unwrap_or(Ok(HttpMethod::GET))
      .map_err(|e| CurlGatewayError::new(
          ErrorKind::Parsing,
          "parse_method",
...
fn parse_url(command: &str) -> Result<String, CurlGatewayError> {
  let url_regex = Regex::new(r"(https?://\S+)").unwrap();
  url_regex
      .captures(command)
      .ok_or_else(|| CurlGatewayError::new(
          ErrorKind::Parsing,
          "parse_url",
          "Missing URL in curl command"
      ))
